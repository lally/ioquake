...
static qboolean within(struct position p,
		   struct region r) {
...
static inline qboolean is_set(bitmap_t map, int i) {
  return (map & (1 << i)) != 0;
}
...
static inline qboolean full_set(bitmap_t map) {
...
double distance(struct position a, struct  position b,
  ...
  qboolean a_in_reg = qfalse;
  qboolean b_in_reg = qfalse;
  int r;
...
int main(int args, char ** argv) {
  ...
  int self_refs = 0;
  qboolean r = makeWaypointTable(&wmap, &regs, NR_POINTS, points);
  ...
  for (i=0; i<SIZE(wmap); ++i) {
    int_vec_t path;
    INIT(path);
    qboolean ret = pathFind(&path, GET(wmap,0).pos, GET(wmap, i).pos,
			&wmap, &regs);
